...
## Question 7.1 Answer
Currently I work for a telecom company as a network engineer. My day to day responsibilities include monitoring servers across the network and ensure they maintain enough capacity to perform without issue. One situation I could quickly point out where exponential smoothing would be appropriate would be in the forecasting of user consumption of the servers on the network. 

There are many ways a forecast can be created and much of it depends on the experience of the team performing the analysis. However, the user consumption forecasts are performed every quarter or 90 days. They include actuals vs projections, and then future expected demand.Using exponential smoothing in our current forecasts would possibly create a more accurate forecast, allowing the organization to plan across the network more efficiently.

In this instance, the data needed would be the user consumption reports across the network. I would expect the value of alpha to be closer to 1, so that the more recent records would play a more important role with the future consumption. No matter what the previous forecasts may show, if the current network is showing an increase in consumption, the network engineers have to increase server efficiency or capacity to match at the minimum. 

## Question 7.2
Using the 20 years of daily high temperature data for Atlanta (July through October) from Question 6.2 (file temps.txt), build and use an exponential smoothing model to help make a judgement of whether the unofficial end of summer has gotten later over the 20 years. 
...
Let's start with single exponential smoothing and see what I can find. In order to set the HoltWinters function to this, I set both Beta and Gamma to False within the function and use the time_data variable I saved the timeseries data to as the x. After running the function, I can print the results by saving them to a variable and running it on a line. I can plot the results as well with the plot function.   
...
To dive deeper into the data, I think I will look into the seasonal coefficients more. For a better view, I wil use the fitted function on the last exponential smoothing with multiplicative seasonality. 
```{r}
test4$fitted
```
